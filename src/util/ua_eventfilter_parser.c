...
create_next_operator_element(UA_Element_List *elements) {
    UA_Parsed_Element_List *element =
        (UA_Parsed_Element_List *)UA_calloc(1, sizeof(UA_Parsed_Element_List));
...
solve_children_references(UA_Parsed_Element_List *temp, UA_Element_List *elements,
                          size_t *position, size_t *nbr_children,
                          char ***child_references) {
    for(size_t i = 0; i < temp->element.oper.nbr_children; i++) {
        if(temp->element.oper.children[i].identifier == ELEMENTOPERAND) {
            UA_Parsed_Element_List *temp_1 = get_element_by_reference(
                elements, &temp->element.oper.children[i].value.element_ref);
            if(temp_1->identifier == parsed_operator) {
                if(temp_1->element.oper.ContentFilterArrayPosition == 0) {
                    (*nbr_children)++;
                    *child_references = (char **)UA_realloc(
                        *child_references, *nbr_children * sizeof(char *));
                ...
                extension.content.decoded.data =
                    &temp_1->element.oper.ContentFilterArrayPosition;
                UA_ExtensionObject_copy(&extension,
...
check_recursion_on_operands(UA_Parsed_Element_List *element, size_t init_element,
                            UA_Element_List *elements, size_t *ctr,
                            UA_StatusCode *status) {
    if(*status != UA_STATUSCODE_GOOD)
        return UA_STATUSCODE_GOOD;
    for(size_t i = 0; i < element->element.oper.nbr_children; i++) {
        if(element->element.oper.children[i].value.extension.content.decoded.type ==
           &UA_TYPES[UA_TYPES_ELEMENTOPERAND]) {
            UA_ElementOperand *temp =
                (UA_ElementOperand *)element->element.oper.children[i]
                    .value.extension.content.decoded.data;
            ...
            (*ctr)++;
            UA_StatusCode retval = check_recursion_on_operands(next_element, init_element,
                                                               elements, ctr, status);
...
static void
handle_filter_structure(char *first_element, UA_Element_List *elements,
                        size_t *position) {
    UA_Parsed_Element_List *temp;
    ...
    while(nbr_children > 0) {
        ...
        for(size_t i = 0; i < temp_ctr; i++) {
            UA_Parsed_Element_List *child =
                get_element_by_reference(elements, &temp_list[i]);
            solve_children_references(child, elements, position, &nbr_children,
                                      &children_list);
            free(temp_list[i]);
        }
        free(temp_list);
        ...
        TAILQ_FOREACH(temp, &elements->head, element_entries) {
            if(temp->identifier == PARSEDOPERAND) {
                if(temp->element.operand.identifier == ELEMENTOPERAND) {
                    temp1 = get_element_by_reference(
                        elements, &temp->element.operand.value.element_ref);
...
clear_linked_list(UA_Element_List *elements) {
    ...
    TAILQ_FOREACH_SAFE(temp, &elements->head, element_entries, temp1) {
        ...
        if(temp->identifier == PARSEDOPERAND) {
            ...
            clear_operand_list(temp->element.oper.children,
...
add_content_filter_element(UA_ContentFilterElement *filterElement,
    ...
    filterElement->filterOperands = (UA_ExtensionObject *)UA_calloc(
        parsedElement->nbr_children, sizeof(UA_ExtensionObject));
...
create_content_filter(UA_Element_List *elements, UA_ContentFilter *filter,
    ...
    TAILQ_FOREACH(temp, &elements->head, element_entries) {
        size_t ctr = 0;
        retval = check_recursion_on_operands(
            temp, temp->element.oper.ContentFilterArrayPosition, elements, &ctr, &status);
        if(retval != UA_STATUSCODE_GOOD) {
            clear_linked_list(elements);
            return retval;
        }
    ...
    filter->elements = (UA_ContentFilterElement *)UA_Array_new(
        filter->elementsSize, &UA_TYPES[UA_TYPES_CONTENTFILTERELEMENT]);
...
add_operator_children(UA_Element_List *global, UA_Parsed_Operator element) {
    ...
    temp->element.oper.children =
        (UA_Parsed_Operand *)UA_calloc(element.nbr_children, sizeof(UA_Parsed_Operand));
...
append_select_clauses(UA_SimpleAttributeOperand **select_clauses, size_t *sao_size,
    ...
    if(*sao_size == 0) {
        *sao_size = 1;
        *select_clauses = (UA_SimpleAttributeOperand *)UA_Array_new(
            *sao_size, &UA_TYPES[UA_TYPES_SIMPLEATTRIBUTEOPERAND]);
        ...
        UA_StatusCode ret_val =
            UA_Array_append((void **)select_clauses, sao_size,
                            (UA_SimpleAttributeOperand *)extension->content.decoded.data,
...
UA_StatusCode
create_literal_operand(char *string, UA_LiteralOperand *lit, UA_StatusCode status) {
    if(status != UA_STATUSCODE_GOOD)
        return status;
    UA_ByteString input_val = UA_String_fromChars(string);
    UA_StatusCode ret_val =
        UA_decodeJson(&input_val, (void *)&lit->value, &UA_TYPES[UA_TYPES_VARIANT], NULL);
    if(ret_val != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
...
void
handle_sao(UA_SimpleAttributeOperand *simple, UA_Parsed_Operand *operand) {
    ...
    operand->value.extension.content.decoded.type =
        &UA_TYPES[UA_TYPES_SIMPLEATTRIBUTEOPERAND];
...
static void
create_element_reference(size_t *branch_nbr, char **ref, const char *ref_identifier) {
    char ref_nbr[128];
...
static void
add_child_references(UA_Parsed_Element_List *element, char **ref_1, char **ref_2) {
    element->element.oper.nbr_children = 2;
    element->element.oper.children =
        (UA_Parsed_Operand *)UA_calloc(2, sizeof(UA_Parsed_Operand));
    for(size_t i = 0; i < 2; i++) {
        element->element.oper.children[i].identifier = ELEMENTOPERAND;
        i == 0
            ? save_string(*ref_1, &element->element.oper.children[0].value.element_ref)
...
static void
set_up_ext_object_from_literal(UA_ExtensionObject *obj, UA_LiteralOperand *value) {
...
add_child_operands(UA_Parsed_Operand *operand_list, size_t operand_list_size,
    ...
    element->filter = oper;
    element->children =
        (UA_Parsed_Operand *)UA_calloc(operand_list_size, sizeof(UA_Parsed_Operand));
    for(size_t i = 0; i < operand_list_size; i++) {
        if(operand_list[i].identifier == ELEMENTOPERAND) {
            save_string(operand_list[i].value.element_ref,
                        &element->children[i].value.element_ref);
...
create_branch_element(UA_Element_List *global, size_t *branch_element_number,
                      UA_FilterOperator filteroperator, char *ref_1, char *ref_2,
                      char **ref) {
    UA_Parsed_Element_List *temp = create_next_operator_element(global);
...
void
handle_for_operator(UA_Element_List *global, size_t *for_operator_reference, char **ref,
                    UA_Parsed_Operator *element) {
    UA_Parsed_Element_List *temp = create_next_operator_element(global);
...
change_element_reference(UA_Element_List *global, char *element_name,
    ...
    temp->ref =
        (char *)UA_realloc(temp->ref, (strlen(new_element_reference) + 1) * sizeof(char));
...
add_in_list_children(UA_Element_List *global, UA_Parsed_Operand *oper) {
    UA_Parsed_Element_List *temp = get_last_element_from_list(global);
    temp->element.oper.nbr_children++;
    temp->element.oper.children = (UA_Parsed_Operand *)UA_realloc(
        temp->element.oper.children,
...
append_string(char **string, char *yytext) {
    ...
    *string = (char *)UA_realloc(*string, new_size * sizeof(char));
    memset((void *)((char *)(*string) + old_size), 0,
...
UA_StatusCode
set_up_variant_from_qname(char *str, UA_Variant *litvalue, UA_StatusCode status) {
    if(status != UA_STATUSCODE_GOOD)
        return status;
    ...
    if(ret_val != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                       "Failed to parse the RelativePath %s",
                       UA_StatusCode_name(ret_val));
        UA_String_clear(&parsed_string);
